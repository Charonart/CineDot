...
export function AiScheduleDraftBanner({
  ...
  return (
    <div className="bg-slate-900 text-white border-b border-slate-800 px-4 py-2.5 shrink-0 shadow-md font-sans select-none">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-2.5">
        {/* Left: Status Pill & Key Indicators */}
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10.5px] font-mono font-medium px-2 py-0.2 rounded-md bg-slate-800 text-slate-300 border border-slate-700">
                {draft_showtimes.length} suất
              </span>
              <span className="text-[10px] font-medium px-2 py-0.2 rounded bg-slate-800/80 text-slate-400">
            ...
            {/* Quick Metrics Strip */}
            <div className="flex items-center gap-2.5 text-xs text-slate-300 mt-0.5 flex-wrap font-medium">
              ...
              <span className="flex items-center gap-1 text-emerald-400 font-mono font-medium">
                ...
                <span>Ước tính: ~{summary.estimated_expected_revenue?.toLocaleString('vi-VN')} đ</span>
              </span>

              {conflictsCount > 0 && (
                <>
                  <span className="text-slate-600">&bull;</span>
                  <span className="flex items-center gap-1 text-rose-400 font-semibold bg-rose-950/80 px-2 py-0.2 rounded border border-rose-800/60 text-[10.5px]">
        ...
        <div className="flex items-center gap-2 self-end lg:self-center shrink-0 flex-wrap">
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer border border-slate-700"
          >
            <Info className="w-3.5 h-3.5 text-[#7C6FE8]" />
            <span>{showExplanation ? 'Ẩn chi tiết' : 'Chi tiết chiến lược'}</span>
          ...
          <button
            ...
            disabled={isApplying}
            className="px-3 py-1.5 rounded-lg hover:bg-rose-950/50 hover:text-rose-300 text-slate-400 text-xs font-medium flex items-center gap-1 transition-colors cursor-pointer border border-slate-800 disabled:opacity-50"
          >
          ...
          <button
            ...
            onClick={handleApply}
            disabled={isApplying}
            className="px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
          >
            {isApplying ? (
              <>
      ...
      {showExplanation && (
        <div className="mt-2.5 p-3 rounded-xl bg-slate-950/80 border border-slate-800 text-xs text-slate-300 flex flex-col gap-1.5 animate-in fade-in duration-150">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-1">
            <div className="flex items-center gap-2 font-semibold text-white">
              <Sparkles className="w-3.5 h-3.5 text-[#7C6FE8]" />
              <span>Phân tích & Tối ưu hóa từ AI:</span>
            </div>
            <span className="text-[10.5px] text-slate-400 font-mono">
              Tối ưu hóa công suất phòng & giờ vàng
            </span>
          </div>

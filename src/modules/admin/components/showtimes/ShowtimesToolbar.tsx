...
export function ShowtimesToolbar({
  ...
  const dateRailRef = React.useRef<HTMLDivElement>(null);

  const scrollDateRail = (direction: 'left' | 'right') => {
    if (dateRailRef.current) {
      const scrollAmount = direction === 'left' ? -260 : 260;
      dateRailRef.current.scrollBy({ left: scrollAmount, behavior: 'smooth' });
    }
  };

  // Scroll active date pill into view smoothly
  React.useEffect(() => {
    if (dateRailRef.current) {
      const activeBtn = dateRailRef.current.querySelector(`[data-date-key="${selectedDateKey}"]`);
      if (activeBtn) {
  ...
  return (
    <div className="bg-slate-50/70 border-b border-gray-200 px-3.5 py-2.5 flex flex-col gap-2 select-none shrink-0 font-sans">
      ...
      {/* Date Rail (Multi-Day Scrollable Strip) */}
      <div className="relative flex items-center gap-1 pt-1 border-t border-gray-200/60">
        {/* Left Scroll Trigger */}
        <button
          onClick={() => scrollDateRail('left')}
          className="p-1 rounded-md bg-white hover:bg-slate-100 text-slate-500 hover:text-slate-800 border border-gray-200 shadow-2xs shrink-0 cursor-pointer"
          title="Cuộn ngày trước"
        >
        ...
        <div
          ref={dateRailRef}
          className="flex items-center gap-1 overflow-x-auto scrollbar-none scroll-smooth flex-1 py-0.5"
        >
          {datePills.map((p) => {
            const isSelected = selectedDateKey === p.key;
            return (
              <button
                key={p.key}
                data-date-key={p.key}
                onClick={() => onSelectDate(p.key)}
                className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer whitespace-nowrap shrink-0 ${
                  isSelected
                    ? 'bg-slate-900 text-white shadow-2xs ring-1 ring-slate-900'
                    : 'text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-100 border border-gray-200/80 shadow-2xs'
                }`}
              >
                {p.label}
              </button>

  ...
  async fetchShowtimeBookingData(showtimeId: string | number): Promise<{
    ...
    try {
      ...
      const showtimeInfo: ShowtimeBookingInfo = {
        ...
        screenType: stData.screen_type || showtimeMeta.screen_type || (stData.format?.toLowerCase().includes('imax') ? 'imax_laser' : stData.format?.toLowerCase().includes('screenx') ? 'screenx' : stData.format?.toLowerCase().includes('dolby') ? 'dolby_cinema' : 'standard_2d'),
        soundTechnology: stData.sound_technology || showtimeMeta.sound_technology || (stData.format?.toLowerCase().includes('atmos') ? 'dolby_atmos' : 'surround_71'),
        screenConfig: stData.screen_config || showtimeMeta.screen_config || seatPayload.screen,
        features: stData.features || showtimeMeta.features || [],

...
export const SeatLegend: React.FC<SeatLegendProps> = ({
  ...
  return (
    <div className="w-full flex flex-col gap-4 py-4 border-t border-gray-200 text-xs font-semibold text-slate-700">
      ...
      {/* 2. Price Tariff Bar */}
      <div className="w-full flex flex-wrap items-center justify-center gap-4 sm:gap-6 pt-3 border-t border-gray-100 text-[11px] text-slate-500 font-medium">
        <span>Bảng giá niêm yết:</span>
        {hasStandard && (
          <span className="flex items-center gap-1">
            <strong className="text-slate-800">Ghế Thường:</strong> {standardPrice.toLocaleString('vi-VN')}đ
          </span>
        )}
        {hasStandard && hasVip && <span className="text-slate-300">•</span>}
        {hasVip && (
          <span className="flex items-center gap-1">
            <strong className="text-[#7C6FE8]">Ghế VIP:</strong> {vipPrice.toLocaleString('vi-VN')}đ
          </span>
        )}
        {(hasStandard || hasVip) && hasSweetbox && <span className="text-slate-300">•</span>}
        {hasSweetbox && (
          <span className="flex items-center gap-1">
            <strong className="text-pink-600">Ghế Đôi Sweetbox:</strong> {sweetboxPrice.toLocaleString('vi-VN')}đ / cặp
          </span>
        )}
      </div>
    </div>

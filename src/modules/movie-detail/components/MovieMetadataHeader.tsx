      ...
      <div className="relative z-20 flex flex-col md:flex-row gap-6 sm:gap-8 items-start pb-8 border-b border-gray-200/80">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <AgeRatingBadge ageRating={movie.ageRating} size="md" variant="pill" showTooltip />
            {movie.country && (
          ...
          {/* Age Warning Notice if restricted */}
          {getAgeWarningNotice(movie.ageRating) && (
            <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-900 text-xs font-medium max-w-2xl">
              <ShieldAlert className="w-4 h-4 text-amber-600 shrink-0" />
              <span>{getAgeWarningNotice(movie.ageRating)}</span>
            </div>
          )}
          ...
          {/* IMDb Rating Score Block */}
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/25 shadow-xs">
              <span className="px-1.5 py-0.5 rounded bg-[#F5C518] text-black font-black text-[11px] tracking-wider uppercase leading-none shadow-2xs">
                IMDb
              </span>
            ...
            {movie.rating > 0 && movie.voteCount > 0 && (
              <span className="text-xs text-gray-500 font-medium">
                ({movie.voteCount.toLocaleString()} lượt đánh giá quốc tế)
              </span>
            )}

            {(movie.imdbUrl || movie.imdbId) && (
              <a
                href={movie.imdbUrl || `https://www.imdb.com/title/${movie.imdbId}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-xs font-semibold text-[#7C6FE8] hover:text-[#685bc7] transition-colors group/imdb"
                title="Xem trang phim chính thức trên IMDb"
              >
                <span>Xem trên IMDb</span>
                <ExternalLink className="w-3 h-3 group-hover/imdb:translate-x-0.5 transition-transform" />
              </a>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            >
              ...
              <span>{movie.status === 'upcoming' ? 'LỊCH CHIẾU & ĐẶT VÉ' : 'MUA VÉ NGAY'}</span>
            </button>

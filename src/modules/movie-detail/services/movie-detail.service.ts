  ...
  try {
    ...
    if (res.data?.success && res.data?.data) {
      ...
      return {
        ...
        director: Array.isArray(m.crew) ? (m.crew.find((c: any) => c.job === 'Director')?.name || '') : (m.director || ''),
        ...
        castMembers: Array.isArray(m.cast) ? m.cast.map((c: any) => ({
          ...
          profileUrl: c.profilePath || c.profileUrl ? imageHelper.getPosterUrl(c.profilePath || c.profileUrl) : undefined,
          order: c.order,
        })) : [],

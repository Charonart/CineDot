...
const DEFAULT_FONT = 'bold 30px Inter';
const DEFAULT_FONT_URL = 'https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap';
...
async function loadFontFromStylesheet(url: string): Promise<string> {
  ...
  if (!FontFaceConstructor) return 'Inter';
...
function createTextTexture(gl: any, text: string, font = 'bold 30px monospace', color = 'black'): { texture: Texture; width: number; height: number } {
...
class Media {
  ...
  onResize(dimensions: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
    if (dimensions.screen) this.screen = dimensions.screen;
    if (dimensions.viewport) {
      this.viewport = dimensions.viewport;
